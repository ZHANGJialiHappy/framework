...
async function showStatistics() {
  ...
  document.getElementById("statistics-title").innerHTML =
    "Bird Quiz statistics for " + userName;

  Promise.all([getAttempts(), getWins(), getLosses(), generateSessionTable()])
    .then((result) => {
      document.getElementById("atempts").innerHTML =
        "Number of attempts: " + result[0];
      document.getElementById("wins").innerHTML =
        "Number of wins: " + result[1];
      document.getElementById("losses").innerHTML =
        "Number of losses: " + result[2];
      document.getElementById("sessions").innerHTML = result[3];
    })
    .catch((error) => {
...
async function generateSessionTable() {
  ...
  let row = sessionsArray.map(makeRow);
  let rows = row.reduce(con, "");
  return `
      <table>
        <tr>
        <th>Session</th>
...
async function generateSessionTable2() {
  let sessionsArray = await getSessions();

  // normal function
  let f = (session) => {
    if (session.isWin) return "Win";
    else return "Loss";
  };
  ...
  let rows = sessionsArray
    ...
    .map((elem, index) => `<tr><td>${index + 1}.</td><td>${elem}</td></tr>`)
